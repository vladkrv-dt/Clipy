...
extension CPYSnippetsEditorWindowController: NSOutlineViewDataSource {
    ...
    func outlineView(_ outlineView: NSOutlineView, validateDrop info: NSDraggingInfo, proposedItem item: Any?, proposedChildIndex index: Int) -> NSDragOperation {
        ...
        guard let draggedData = unarchivedDraggedData(from: data) else { return NSDragOperation() }
    ...
    func outlineView(_ outlineView: NSOutlineView, acceptDrop info: NSDraggingInfo, item: Any?, childIndex index: Int) -> Bool {
        ...
        guard let draggedData = unarchivedDraggedData(from: data) else { return false }
...
// MARK: - Helpers
private extension CPYSnippetsEditorWindowController {
    func unarchivedDraggedData(from data: Data) -> CPYDraggedData? {
        if #available(macOS 10.13, *) {
            return try? NSKeyedUnarchiver.unarchivedObject(ofClass: CPYDraggedData.self, from: data)
        }
        return NSKeyedUnarchiver.unarchiveObject(with: data) as? CPYDraggedData
    }
}
